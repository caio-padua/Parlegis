...
- Login/access screen (`access-gateway.tsx`): standalone full-screen dark petróleo page with NO site header/footer (excluded via route check in `Layout.tsx`). Brand lockup: prominent CJ symbol (`cicero_symbol.png`), "Cícero João" in solid matte gold #CBA13A with a strong gold glow (no gradient-clip — clips the "J" descender), "Vereador" between two filete lines, gold italic quote "Servir com sabedoria, conduzir com firmeza." Backdrop: brushed-gold 3D Sorocaba cathedral (`sorocaba_monument_gold.png`, generated, transparent) rising from a warm gold "luz no infinito" horizon glow — the user EXPLICITLY wants the Sorocaba monument here (overrides the earlier "no city image" rule). Auth form in a framed gold-ribbon panel with a CUSTOM header (gold key emblem + "Acesso ao Gabinete"); Clerk's own header is hidden via appearance (`header/headerTitle/headerSubtitle: "hidden!"`). System credits PARLEGIS®/PADCON Platform® (the user's companies) bottom-RIGHT, slow-pulsing status dots (`.pulse-dot` in `index.css`, yellow Acesso Restrito + green Entrada Segura, in sync), bottom band of boxed security badges with Portuguese meanings. Do not reintroduce site chrome on auth pages.
